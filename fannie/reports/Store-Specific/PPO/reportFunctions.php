...
$board_discount = 15;
$MAD_discount = 10;
$need_based_discount = 4;
$SSDdiscount = 10;
$NMD_discount = 10;
$invDept = 20; 					// upper limit of inventory dept numbers

$dbChangeDate = '2009-10-08'; 	// Date of discount recording change...
$dbNewDate = '2009-10-09'; 		// Date of first day with new discount reporting...
// 
// global $staff_discount;
// global $volunteer_discount;
// global $board_discount;
// global $MAD_discount;
// global $need_based_discount;
// global $dbChangeDate;
// global $dbNewDate;
// global $SSDdiscount;
// global $NMD_discount;
...
// 		AND department >= 30 AND department <= 44 AND department <> 37
// 		AND trans_subtype NOT IN ('IC', 'MC', 'CP')
// 		AND trans_status <> 'X'
// 		AND emp_no <> 9999";
// 
...
function deptTotals ($title,$gross,$table,$date1,$date2,$in,$bgcolor) {
	...
	echo "<table><tr><td><p><b>" . $title . " Subtotal: </b></p></td><td><p>" . money_format('%n',$tot) . "</p></td><td><p> (". number_format($pct,2) ."%)</p></td></tr></table>\n";
...
function staff_total($table,$date1,$date2) {
	...
	// 
	// $staff_total = $SSDdiscount2 + $staff_total;
...
function NMDdiscount($table,$date1,$date2) {
	...
	$NMDQ = "SELECT (SUM(unitPrice)) AS NMD_total
		FROM " . DB_LOGNAME . ".$table
		WHERE date(datetime) >= '$date1' AND date(datetime) <= '$date2'
		AND card_no = 99910
		AND voided = 5
...
function MADcoupon($table,$date1,$date2) {
	...
	// }
	$MADDQ = "SELECT SUM(total) as tot, COUNT(total) as ct
		FROM " . DB_LOGNAME . ".$table
		WHERE DATE(datetime) BETWEEN '$date1' AND '$date2'
		AND (upc = 'MADDISCOUNT' OR upc = '10MA')
		AND emp_no <> 9999
		AND trans_status <> 'X'";
	$MADDR = mysql_query($MADDQ);
	list($MADcoupon, $MADD_num) = mysql_fetch_row($MADDR);
	
	$MADcoupon = (is_null($MADcoupon) ? 0 : $MADcoupon);
	$MAD_num = (is_null($MADD_num) ? 0 : $MADD_num);
...
function SSDdiscount($table,$date1,$date2) {
	...
	// 	
		$SSDDQ = "SELECT SUM(total) as tot, COUNT(total) as ct
			FROM " . DB_LOGNAME . ".$table
			WHERE DATE(datetime) BETWEEN '$date1' AND '$date2'
			AND (upc = 'SPECIALDISC' OR upc = '10SSDD')
			AND emp_no <> 9999
			AND trans_status <> 'X'";
		$SSDDR = mysql_query($SSDDQ);
...
// 		AND trans_status <> 'X'";
// 
// 	$mktR = mysql_query($mktQ);
// 	list($mkttot) = mysql_fetch_row($mktR);
// 	$mkttot = (is_null($mkttot) ? 0 : $mkttot);
// 	
// 	return $mkttot;
// }
...
function net_total($table, $date1, $date2) {
	...
	extract(SSDdiscount($table, $date1, $date2));
	$mktebt = marketEBT_total($table, $date1, $date2);

	
	$net = $gross + $hash + $totalDisc /*- $SSDdiscount2*/ + $pt_total + $coupons + $strchg + $RA + $mktebt;
	//
	//	NOTE:  SSDdiscount2 re-added to correct glitch: STAFF discount and Staff Special Discount are colliding
	//
	return $net;
